...
public class ImgUploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		case "notice": {
			...
			parentFolder = "uploadnew/school" + userInfo.getSID() + "/notice";
			if (!EmptyUtils.stringIsEmpty(noticeType)) {
				parentFolder += "/" + noticeType;
			}
		}
			break;
		case "schoolinfo": {
			parentFolder = "uploadnew/school" + userInfo.getSID() + "/schoolinfo";
		}
			break;
		case "album": {
			parentFolder = "uploadnew/school" + userInfo.getSID() + "/album";
			String albumType = request.getParameter("type");
			String albumId = request.getParameter("albumId");
			if (!EmptyUtils.stringIsEmpty(albumType)) {
				parentFolder += "/" + albumType;
			}
			if (!EmptyUtils.stringIsEmpty(albumId)) {
				parentFolder += "/" + albumId;
			}
		}
			break;
		case "userinfo": {
			parentFolder = "uploadnew/school" + userInfo.getSID() + "/userinfo/" + userInfo.getUSERID();
		}
			break;
		case "lefsketch": {
			parentFolder = "uploadnew/school" + userInfo.getSID() + "/lefsketch/" + userInfo.getUSERID();
		}
			break;
		default:
			parentFolder = "uploadnew";
			break;
		}
		...
		for (FileItem item : list) {
			...
			if (!item.isFormField()) {
				...
				String filename = format + ran + System.currentTimeMillis() + suffix;
