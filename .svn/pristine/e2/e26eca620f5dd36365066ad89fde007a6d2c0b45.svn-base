...
//        iframeWidth: '100%',
    ...
    _pop = function(id, e, mouse) {
        ...
        var remove = e.attr('id') ? _createMenu('remove', 'glyphicon-remove-circle', settings.local.closeThisLabel) : '';
        ...
        }).append(refresh).append(remove).append(_createMenu('remove-circle', 'glyphicon-remove-circle', settings.local.closeOtherLabel)).append(left).append(right).append(_createMenu('remove-all', 'glyphicon-remove', settings.local.closeAllLabel));
        popHtml.css({//添加样式
            'top': mouse.pageY,
            'left': mouse.pageX,
        ...
        $('ul.rightMenu a[data-right=remove-circle]').on('click', function() {
            ...
            target.find('li').each(function() {
                var id = $(this).attr('id');
                if (id && id != 'tab_tab_HXT_HOME' && id != 'tab_' + tab_id) {//首页不能关闭
                    $.addtabs.close({
        ...
        $('ul.rightMenu a[data-right=remove-all]').on('click', function() {
            ...
            target.find('li').each(function() {
                var id = $(this).attr('id');
                if (id && id != 'tab_tab_HXT_HOME') {//首页不能关闭
                    $.addtabs.close({
                        "id": $(this).children('a').attr('aria-controls')
                    });
                }
    ...
    $.addtabs.add = function(opts) {
        ...
        if (tab_li.find('#tab_' + id).length < 1) {
            ...
            if (settings.close) {
                title.append($('<i>', {
                    'class': 'close-tab glyphicon glyphicon-remove-circle'
                }));
            }
            //创建新TAB的内容
            var content = $('<div>', {
        ...
        if (opts.content) {
        	...
        	var ifr = '<iframe width="100%" height="100%">' ;
