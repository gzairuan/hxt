...
/**
 * 用户管理初始化信息 Servlet implementation class UserInitServlet
 * 
...
public class UserInitServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (optFlag == null) {
			...
			if (optFlag.equals("init")) {// 初始化Table操作
				...
				String uschool = user.getSID()==0?request.getParameter("uschool"):user.getSID()+"";// 学校
				String cid = request.getParameter("cid");// 学校
				String uname = request.getParameter("uname");// 用户名
				String urname = request.getParameter("urname");// 电话
				...
				if (!EmptyUtils.stringIsEmpty(uschool)&&uschool!="0") {
					map.put("SID", uschool);
				}
				...
				if (!EmptyUtils.stringIsEmpty(urname)) {
					map.put("REALNAME", urname);
				}
				...
				String userJson = getUserInfo(request,user.getSID());
	...
	private String getUserInfo(HttpServletRequest request,int sid) {
		...
		if(!EmptyUtils.stringIsEmpty(uSchoolId)){
			sid = TransformUtils.transformInt(uSchoolId);
		}
		userInfo.setSID(sid);
