						...
						var domli = '<li id="photoLiId">'+
										'<div class="post-container">'+
											'<div class="post-option operate-role" style="display:none;">' +
												'<ul class="list-options">'+ //操作 
													'<li class="edit-role" style="display:none;">' +
														'<a type="button" data-toggle="modal" data-target="#addAlbumModal" data-flag="editPhoto" data-id="'+ photoid +'" data-albumid="'+ albumid +'" data-photodsc="'+ photodsc +'"> ' +
																'<i class="fa fa-edit"></i> <span>修改</span></a>' +
													'</li>'+
													/*'<li>' +
														'<a href="#"> <i class="fa  fa-download"></i> <span>移动</span></a>' +
...
$('#addAlbumModal').on('show.bs.modal', function(event){
	...
	switch (opt){
		...
		case "editPhoto": //修改照片
			...
			var photoDesc = btn.data("photodsc"), //相册描述
				photoid = btn.data("id"),
				albumid = btn.data("albumid"); 
			$('#albumDesc').val(photoDesc); //填充值
			btnSave.removeAttr('onclick');
			btnSave.attr('onclick', 'editPhoto('+ photoid +', "'+albumid+'")');
		break;
